...
export const useHoursPage = () => {
  ...
  // Stats configuration
  const totalHours = worklog.reduce(
    (sum, entry) => sum + (entry.hours.hours || 0),
    0
  );
  const approvedHours = worklog
    .filter((e) => e.status === 'approved')
    .reduce((sum, entry) => sum + (entry.hours.hours || 0), 0);
  const pendingHours = worklog
  ...
  const filteredEntries = worklog.filter((entry) => {
    const matchesSearch =
      entry.notes
        .toLowerCase()
        .includes(searchAndFilter.searchTerm.toLowerCase()) ||
    ...
    const matchesFilter =
      ...
      entry.status === searchAndFilter.filters.status;
