...
export const Navigation = () => {
  ...
  const navItems = useMemo(
    () => [
      ...
      },
      {
        ...
        href: '/volunteers',
        icon: Users,
        roles: ROUTE_PERMISSIONS.VOLUNTEERS,
      },
      {
        name: 'Activities',
        href: '/activities',
        icon: Activity,
        roles: ROUTE_PERMISSIONS.ACTIVITIES,
      },
      // { name: "Events", href: "/events", icon: Calendar, roles: [] },
      {
        ...
        href: '/hours',
        icon: Clock,
        roles: ROUTE_PERMISSIONS.HOURS,
      },
      {
        name: 'Resources',
        href: '/resources',
        icon: FileText,
        roles: ROUTE_PERMISSIONS.RESOURCES,
      },
      {
        name: 'Notifications',
        href: '/notifications',
        icon: Bell,
        roles: ROUTE_PERMISSIONS.NOTIFICATIONS,
      },
  ...
  const allowedNavItems = useMemo(
    () => navItems.filter((item) => canAccessRoute(user, item.roles)),
    [navItems, user]
  );
              ...
              {allowedNavItems.map((item) => {
                ...
                return (
                  <NavLink
                    ...
                    to={item.href}
                    className={({ isActive }) =>
                      `px-2 md:px-3 py-2 rounded-md text-sm font-medium flex items-center md:space-x-2 transition-smooth ${
                        isActive
                  ...
                  >
                    ...
                    <span
                      className={`hidden ${isVolunteer(user?.role) ? 'lg:inline' : 'xl:inline'}`}
                    >
                      {item.name}
                    </span>
                  </NavLink>
              ...
              {allowedNavItems.map((item) => {
                ...
                return (
                  <NavLink
                    ...
                    onClick={() => setIsOpen(false)}
                    className={({ isActive }) =>
                      `px-3 py-3 rounded-md text-base font-medium flex items-center space-x-3 transition-smooth ${
                        isActive
            ...
            <div className="px-4 py-4 border-t border-border">
              <div className="space-y-2">
                <NavLink
                  ...
                  onClick={() => setIsOpen(false)}
                  className={({ isActive }) =>
                    `w-full flex items-center px-3 py-3 rounded-md text-base font-medium justify-start space-x-3 transition-smooth ${
                      isActive
                        ? 'bg-gradient-primary text-primary-foreground'
                        : 'text-muted-foreground hover:text-foreground hover:bg-accent'
                    }`
