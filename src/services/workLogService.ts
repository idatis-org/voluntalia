...
export const getWorkLogsByUser = async (): Promise<WorkLog[]> => {
  ...
  const normalized = camelizeKeys<WorkLog[]>(raw).map((w) => ({
    ...
    weekStart:
      w.weekStart ??
      ((w as unknown as Record<string, unknown>)['week_start'] as
        | string
        | undefined),
    activityTitle:
      w.activityTitle ??
      ((w as unknown as Record<string, unknown>)['activity_title'] as
        | string
        | undefined),
  }));
...
export const createWorkLog = async (
  ...
  created.weekStart =
    created.weekStart ??
    ((raw as Record<string, unknown>)['week_start'] as string | undefined);
  created.activityTitle =
    created.activityTitle ??
    ((raw as Record<string, unknown>)['activity_title'] as string | undefined);
  return created;
};
...
export const updateWorklog = async (
  id: string,
  worklogData: Partial<UpdateWorkLogDTO>
): Promise<WorkLog> => {
  const payload = snakeifyKeys(worklogData);
