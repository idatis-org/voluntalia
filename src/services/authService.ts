...
export const loginUser = async ({
  ...
  const createdAtFromRaw =
    rawUser && typeof rawUser === 'object'
      ? ((rawUser as Record<string, unknown>)['created_at'] as
          | string
          | undefined)
      : undefined;
  ...
  const accessToken = (data['accessToken'] ?? data['access_token']) as
    | string
    | undefined;
  const refreshToken = (data['refreshToken'] ?? data['refresh_token']) as
    | string
    | undefined;
